...
import 'package:easy_idb/easy_idb.dart';
...
class EasyIdbDatabase extends DatabaseApi {
  static const int version = 6;
  final String name;
  late IdbCollection _collection;
  late idb.IdbFactory? factory;
  late IdbBox<String> _clientBox;
  late IdbBox<Map> _accountDataBox;
  late IdbBox<Map> _roomsBox;
  late IdbBox<Map> _toDeviceQueueBox;

  /// Key is a tuple as TupleKey(roomId, type) where stateKey can be
  /// an empty string.
  late IdbBox<Map> _roomStateBox;

  /// Key is a tuple as TupleKey(roomId, userId)
  late IdbBox<Map> _roomMembersBox;

  /// Key is a tuple as TupleKey(roomId, type)
  late IdbBox<Map> _roomAccountDataBox;
  late IdbBox<Map> _inboundGroupSessionsBox;
  late IdbBox<Map> _outboundGroupSessionsBox;
  late IdbBox<Map> _olmSessionsBox;

  /// Key is a tuple as TupleKey(userId, deviceId)
  late IdbBox<Map> _userDeviceKeysBox;

  /// Key is the user ID as a String
  late IdbBox<bool> _userDeviceKeysOutdatedBox;

  /// Key is a tuple as TupleKey(userId, publicKey)
  late IdbBox<Map> _userCrossSigningKeysBox;
  late IdbBox<Map> _ssssCacheBox;
  late IdbBox<Map> _presencesBox;

  /// Key is a tuple as Multikey(roomId, fragmentId) while the default
  /// fragmentId is an empty String
  late IdbBox<List> _timelineFragmentsBox;

  /// Key is a tuple as TupleKey(roomId, eventId)
  late IdbBox<Map> _eventsBox;

  /// Key is a tuple as TupleKey(userId, deviceId)
  late IdbBox<String> _seenDeviceIdsBox;

  late IdbBox<String> _seenDeviceKeysBox;
  ...
  EasyIdbDatabase(this.name, {this.factory});

  @override
  int get maxFileSize => 0;

  Future<void> open() async {
    _collection = await IdbCollection.open(
      name,
      {
        _clientBoxName,
        _accountDataBoxName,

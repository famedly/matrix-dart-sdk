...
class FluffyBoxDatabase extends DatabaseApi {
  ...
  final String name;
  final Future<Database> Function()? openSqlDatabase;
  ...
  FluffyBoxDatabase(this.name, {this.openSqlDatabase});
  ...
  Future<void> open() async {
    _collection = await BoxCollection.open(
      ...
      sqfliteDatabase: await openSqlDatabase?.call(),
    );
    _clientBox = _collection.openBox(
      _clientBoxName,
    );
    _accountDataBox = _collection.openBox(
      _accountDataBoxName,
    );
    _roomsBox = _collection.openBox(
      _roomsBoxName,
    );
  ...
  Future<void> _migrateFromVersion(int currentVersion) async {
    ...
    if (version == 5) {
      final keys = await _userDeviceKeysBox.getAllKeys();
  ...
  Future<void> clear() async {
    Logs().i('Clear and close store...');
    await _actionOnAllBoxes((box) => box.clear());
    return;
  }
        ...
        for (final userId in deviceKeysOutdated) {
          ...
          res[userId] = DeviceKeysList.fromDbJson(
              ...
              await Future.wait(deviceKeysBoxKeys.map((key) async =>
                  copyMap((await _userDeviceKeysBox.get(key))!))),
              await Future.wait(crossSigningKeysBoxKeys.map((key) async =>
                  copyMap((await _userCrossSigningKeysBox.get(key))!))),
              client);
        }
        return res;
      });

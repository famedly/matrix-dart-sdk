...
class LiveKitBackend extends CallBackend {
  ...
  int _getNewEncryptionKeyIndex(GroupCallSession groupCall) {
    Logs().v(
        '[_getNewEncryptionKeyIndex] using keyRingSize ${groupCall.voip.keyRingSize}');
    final newIndex = _indexCounter % groupCall.voip.keyRingSize;
    _indexCounter++;
    return newIndex;
  }
  ...
  Future<void> _makeNewSenderKey(
    ...
    final keyIndex = _getNewEncryptionKeyIndex(groupCall);
  ...
  Future<void> _setEncryptionKey(
    ...
    if (delayBeforeUsingKeyOurself) {
      ...
      final useKeyTimeout =
          Future.delayed(groupCall.voip.useKeyDelay, () async {
        Logs().i(
  ...
  Future<void> onLeftParticipant(
    ...
    _memberLeaveEncKeyRotateDebounceTimer =
        Timer(groupCall.voip.makeKeyDelay, () async {

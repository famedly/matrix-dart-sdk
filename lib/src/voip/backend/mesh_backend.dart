...
class MeshBackend extends CallBackend {
  ...
  void _onIncomingCallSetup(GroupCallSession groupCall, CallSession newCall) {
    // The incoming calls may be for another room, which we will ignore.
    if (newCall.room.id != groupCall.room.id) return;

    if (newCall.state != CallState.kRinging) {
      Logs().w(
        '[_onIncomingCallSetup] Incoming call no longer in ringing state. Ignoring.',
      );
      return;
    }
    ...
    if (newCall.groupCallId == null ||
      ...
      Logs().v(
        '[_onIncomingCallSetup] Incoming call with groupCallId ${newCall.groupCallId} ignored because it doesn\'t match the current group call',
      );
      return;
    }
    ...
    Logs().v(
      '[_onIncomingCallSetup] GroupCallSession: incoming call from: ${newCall.remoteUserId}${newCall.remoteDeviceId}${newCall.remotePartyId}',
    );

    _registerListenersBeforeCallAdd(newCall);
  }

  Future<void> _onIncomingCallStart(
    GroupCallSession groupCall,
    CallSession newCall,
  ) async {
    // The incoming calls may be for another room, which we will ignore.
    ...
    if (newCall.state != CallState.kRinging) {
      Logs().w(
        '[_onIncomingCallStart] Incoming call no longer in ringing state. Ignoring.',
      );
      return;
    }
    ...
    if (newCall.groupCallId == null ||
        newCall.groupCallId != groupCall.groupCallId) {
      Logs().v(
        '[_onIncomingCallStart] Incoming call with groupCallId ${newCall.groupCallId} ignored because it doesn\'t match the current group call',
      );
    ...
    Logs().v(
      '[_onIncomingCallStart] GroupCallSession: incoming call from: ${newCall.remoteUserId}${newCall.remoteDeviceId}${newCall.remotePartyId}',
    );
  ...
  Future<void> setupP2PCallsWithExistingMembers(
    ...
    for (final call in _callSessions) {
      _onIncomingCallSetup(groupCall, call);
      await _onIncomingCallStart(groupCall, call);
    }

    _callSetupSubscription = groupCall.voip.onIncomingCallSetup.stream.listen(
      (newCall) => _onIncomingCallSetup(groupCall, newCall),
    );

    _callStartSubscription = groupCall.voip.onIncomingCallStart.stream.listen(
      (newCall) => _onIncomingCallStart(groupCall, newCall),
    );

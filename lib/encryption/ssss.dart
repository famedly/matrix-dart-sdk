...
class SSSS {
  ...
  Future<void> request(String type, [List<DeviceKeys>? devices]) async {
    ...
    if (devices == null || devices.isEmpty) {
      ...
      if (!ownKeys.containsKey(client.userID)) {
        Logs().w('[SSSS] User does not have any devices');
        return;
      }
      devices = ownKeys[client.userID]!.deviceKeys.values.toList();
    }
  ...
  Future<void> handleToDeviceEvent(ToDeviceEvent event) async {
    if (event.type == EventTypes.SecretRequest) {
      ...
      final device = ownKeys[client.userID]!
...
class OpenSSSS {
  ...
  Future<void> _postUnlock() async {
    ...
    final ownKeys = await ssss.client.fetchUserDeviceKeysLists({
      ssss.client.userID!,
    });
    if (ssss.encryption.crossSigning.enabled &&
        ownKeys[ssss.client.userID]?.masterKey != null &&
        (ssss
                .keyIdsFromType(EventTypes.CrossSigningMasterKey)
                ?.contains(keyId) ??
            false) &&
        (await ssss.client.isUnknownSession ||
            ownKeys[ssss.client.userID]!.masterKey?.directVerified != true)) {
      try {

...
class CrossSigning {
  ...
  CrossSigning(this.encryption) {
    encryption.ssss.setValidator(EventTypes.CrossSigningSelfSigning, (
      ...
      try {
        ...
        return keyObj.publicKey.toBase64() ==
            ownKeys[client.userID]!.selfSigningKey!.ed25519Key;
      } catch (_) {
        return false;
      }
    });
    encryption.ssss.setValidator(EventTypes.CrossSigningUserSigning, (
      ...
      try {
        ...
        final ownKeys = await client.fetchUserDeviceKeysLists({client.userID!});
        return keyObj.publicKey.toBase64() ==
            ownKeys[client.userID]!.userSigningKey!.ed25519Key;
      } catch (_) {
        return false;
      }
    });
  ...
  Future<void> selfSign({
    ...
    final ownKeys = await client.fetchUserDeviceKeysLists({client.userID!});
    final userDeviceKeys = ownKeys[client.userID]?.deviceKeys[client.deviceID];
    if (masterPubkey == null || userDeviceKeys == null) {
      throw Exception('Master or user keys not found');
    }
    final masterKey = ownKeys[client.userID]?.masterKey;
  ...
  Future<void> sign(List<SignableKey> keys) async {
    ...
    final ownKeys = await client.fetchUserDeviceKeysLists({client.userID!});
    final userKeys = ownKeys[client.userID];
    if (userKeys == null) {

  ...
  group('Room', () {
    ...
    test('cancelSend because EventTooLarge in postLoaded room', () async {
      ...
      try {
        ...
        await room.client.database?.cancelSend(room.lastEvent!);
      }

      // becase room is postLoaded, it can fall back to the last visible event
      expect(room.lastEvent?.eventId, 'older_event');
      expect(room.lastEvent?.body, 'older_event');
    });

    test('cancelSend because EventTooLarge in NOT postLoaded room', () async {
      final id = '!not_postLoaded';
      ...
      expect(newRoom.partial, true);

      // first fake joined sync, rest are automatically done with sendTextEvent
      await updateLastEvent(
        Event(
          senderId: '@test:example.com',
          type: 'm.room.message',
      ...
      expect(newRoom.id, '!not_postLoaded');
      await newRoom.sendTextEvent(
        'not_postLoaded',
        txid: 'not_postLoaded',
      );
      expect(newRoom.partial, true);
      expect(newRoom.lastEvent?.body, 'not_postLoaded');
      expect(newRoom.lastEvent?.eventId, 'not_postLoaded');
      ...
      try {
        ...
        expect(e.runtimeType, EventTooLarge);
        expect(newRoom.lastEvent?.eventId, 'event_too_large');
        expect(newRoom.lastEvent?.status, EventStatus.error);
        // force null because except would have caught it anyway
        await newRoom.client.database?.cancelSend(newRoom.lastEvent!);
      }

      // becase room isn't postLoaded, it stays at redacted
      expect(newRoom.lastEvent?.eventId, 'event_too_large');
      expect(newRoom.lastEvent?.body, 'Redacted');
    });

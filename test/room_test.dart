...
void main() {
  ...
  group('Room', () {
    ...
    test('setAvatar', () async {
      ...
      final resp = await room.setAvatar(testFile);
      expect(resp, 'YUwRidLecu:example.com');
    });

    test('sendEvent', () async {
      final resp = await room.sendEvent(
    ...
    test('sendEvent', () async {
      ...
      final resp = await room.sendTextEvent('Hello world', txid: 'testtxid');
      expect(resp?.startsWith('\$event'), true);
      final entry = FakeMatrixApi.calledEndpoints.entries
          .firstWhere((p) => p.key.contains('/send/m.room.message/'));
      final content = json.decode(entry.value.first as String);
      expect(content, {
        'body': 'Hello world',
        'msgtype': 'm.text',
      });
    ...
    test('send reply', () async {
      ...
      var entry = FakeMatrixApi.calledEndpoints.entries
          .firstWhere((p) => p.key.contains('/send/m.room.message/'));
      var content = json.decode(entry.value.first as String);
      expect(content, {
      ...
      entry = FakeMatrixApi.calledEndpoints.entries
          .firstWhere((p) => p.key.contains('/send/m.room.message/'));
      content = json.decode(entry.value.first as String);
      expect(content, {
        'body':
    ...
    test('send location', () async {
      ...
      final resp = await room.sendLocation(body, geoUri, txid: 'testtxid');
      expect(resp?.startsWith('\$event'), true);

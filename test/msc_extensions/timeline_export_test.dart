...
void main() {
  group('TimelineExportExtension', () {
    ...
    group('pagination handling', () {
      test('handles database pagination with large event sets', () async {
        final manyDbEvents = createMockEvents(
          count: 150,
          startTime: now.subtract(const Duration(hours: 5)),
          room: room,
        );

        client.dbEvents = manyDbEvents;
        timeline = Timeline(
          room: room,
          chunk: TimelineChunk(events: manyDbEvents.take(1).toList()),
        );
        ...
        await for (final result in timeline.export()) {
          ...
          if (result is ExportProgress &&
              result.source == ExportSource.database) {
            progressUpdates++;
        ...
        expect(progressUpdates, equals(2));
        expect(
          complete.events.map((e) => e.eventId).toSet(),
          manyDbEvents.map((e) => e.eventId).toSet(),
        );
      });

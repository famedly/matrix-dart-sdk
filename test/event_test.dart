...
void main() {
  ...
  group('Event', () {
    ...
    test('Test all EventTypes', () async {
      ...
      jsonObj['content'] = json.decode(jsonObj['content'] as String);

      (jsonObj['content'] as Map<String, Object?>).remove('m.relates_to');
      (jsonObj['content'] as Map<String, Object?>)['msgtype'] = 'm.notice';
      event = Event.fromJson(jsonObj, room);
      expect(event.messageType, MessageTypes.Notice);

      (jsonObj['content'] as Map<String, Object?>)['msgtype'] = 'm.emote';
      event = Event.fromJson(jsonObj, room);
      expect(event.messageType, MessageTypes.Emote);

      (jsonObj['content'] as Map<String, Object?>)['msgtype'] = 'm.image';
      event = Event.fromJson(jsonObj, room);
      expect(event.messageType, MessageTypes.Image);

      (jsonObj['content'] as Map<String, Object?>)['msgtype'] = 'm.video';
      event = Event.fromJson(jsonObj, room);
      expect(event.messageType, MessageTypes.Video);

      (jsonObj['content'] as Map<String, Object?>)['msgtype'] = 'm.audio';
      event = Event.fromJson(jsonObj, room);
      expect(event.messageType, MessageTypes.Audio);

      (jsonObj['content'] as Map<String, Object?>)['msgtype'] = 'm.file';
      event = Event.fromJson(jsonObj, room);
      expect(event.messageType, MessageTypes.File);

      (jsonObj['content'] as Map<String, Object?>)['msgtype'] = 'm.location';
      event = Event.fromJson(jsonObj, room);
      expect(event.messageType, MessageTypes.Location);

      jsonObj['type'] = 'm.sticker';
      (jsonObj['content'] as Map<String, Object?>)['msgtype'] = null;
      event = Event.fromJson(jsonObj, room);
      expect(event.messageType, MessageTypes.Sticker);

      jsonObj['type'] = 'm.room.message';
      (jsonObj['content'] as Map<String, Object?>)['msgtype'] = 'm.text';
      (jsonObj['content'] as Map<String, Object?>)['m.relates_to'] =
          <String, Object?>{};
      ((jsonObj['content'] as Map<String, Object?>)['m.relates_to']
          as Map<String, Object?>)['m.in_reply_to'] = {
        'event_id': '1234',
      };
      event = Event.fromJson(jsonObj, room);
    ...
    test('relationship types', () async {
      ...
      ((jsonObj['content'] as Map<String, Object?>)['m.relates_to']
          as Map<String, Object?>)['rel_type'] = 'm.annotation';
      event = Event.fromJson(jsonObj, room);
    ...
    test('redact', () async {
      final redactJsonObj = Map<String, Object?>.from(jsonObj);
      final testTypes = [
    ...
    test('attachments', () async {
      ...
      expect(
        event.getAttachmentUrl(getThumbnail: true).toString(),
      ...
      expect(
        event.getAttachmentUrl(useThumbnailMxcUrl: true).toString(),

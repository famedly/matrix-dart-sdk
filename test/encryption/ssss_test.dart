...
void main() {
  group('SSSS', tags: 'olm', () {
    ...
    test('createKey', () async {
      // with passphrase
      var newKey = await client.encryption!.ssss.createKey('test', 'key_name');
    ...
    test(
      'migrateSecretsToKey strips non-allowed keys when requested',
      () async {
    ...
    test('migrateSecretsToKey without migrated types does not strip', () async {
      ...
      final staleKey = await ssss.createKey(
        'test-passphrase-old',
        'passphrase-old',
      );
      ...
      expect(analyzed, isNotNull);
      expect(analyzed!.contains(passphraseKey.keyId), true);
      expect(analyzed.contains(staleKey.keyId), true);
    });
    ...
    test(
      ...
      () async {
        ...
        final encrypted = Map<String, Object?>.from(
          client.accountData[secretType]!.content.tryGetMap<String, Object?>(
            'encrypted',
          )!,
        );
    ...
    test('keyIdForNamedSecretStorageKey prefers key used in encrypted secrets '
        'when names collide', () async {
      final ssss = client.encryption!.ssss;
      final defaultKeyId = ssss.defaultKeyId!;
      final defaultKey = ssss.open(defaultKeyId);
      await defaultKey.unlock(recoveryKey: ssssKey);
